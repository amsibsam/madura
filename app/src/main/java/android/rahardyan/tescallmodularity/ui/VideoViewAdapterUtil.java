...
public class VideoViewAdapterUtil {
    ...
    public static void composeDataItem1(final ArrayList<UserStatusData> users, HashMap<Integer, SoftReference<SurfaceView>> uids, int localUid) {
        for (HashMap.Entry<Integer, SoftReference<SurfaceView>> entry : uids.entrySet()) {
            if (DEBUG) {
                Log.d("VideViewAdapterUtil", "composeDataItem1 " + (entry.getKey() & 0xFFFFFFFFL) + " " + (localUid & 0xFFFFFFFFL) + " " + users.size() + " " + entry.getValue());
            }
    ...
    private static void removeNotExisted(ArrayList<UserStatusData> users, HashMap<Integer, SoftReference<SurfaceView>> uids, int localUid) {
        if (DEBUG) {
            Log.d("VideViewAdapterUtil", "removeNotExisted all " + uids + " " + users.size());
        }
        Iterator<UserStatusData> it = users.iterator();
        while (it.hasNext()) {
            UserStatusData user = it.next();
            if (DEBUG) {
                Log.d("VideViewAdapterUtil", "removeNotExisted " + user + " " + localUid);
            }
    ...
    public static void composeDataItem(final ArrayList<UserStatusData> users, HashMap<Integer, SoftReference<SurfaceView>> uids,
        ...
        for (HashMap.Entry<Integer, SoftReference<SurfaceView>> entry : uids.entrySet()) {
            ...
            if (DEBUG) {
                Log.d("VideViewAdapterUtil", "composeDataItem " + users + " " + entry + " " + (localUid & 0XFFFFFFFFL) + " " + s + " " + v + " " + i + " " + local + " " + (uid & 0XFFFFFFFFL) + " " + (uidExcepted & 0XFFFFFFFFL));
            }
    ...
    public static void renderExtraData(Context context, UserStatusData user, VideoUserStatusHolder myHolder) {
        if (DEBUG) {
            Log.d("VideViewAdapterUtil", "renderExtraData " + user + " " + myHolder);
        }
